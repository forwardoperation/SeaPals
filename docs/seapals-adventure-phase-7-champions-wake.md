...
| Shared regression | Adventure content, progression, session, travel, storage, world, deck, Field Note journal, and Simulator tests | Existing save/travel/encounter systems remain compatible with the complete campaign; Stunned, Lionfish ownership and attack removal, Cookie Cutter fallback, per-attack Ensnare, Green Sea Turtle healing, and Deep Sea Jelly Flashing Alarm have controller parity. | Lionfish's unidentified specialized-Support remover, external review, or human playability. |
...
Phase 7 uses the existing local voyage-slot identifier and local-save adapter. It adds no account requirement, player name, email field, chat, user-generated content, public profile, cloud save, or tournament submission. That is a data-minimizing design observation, not a legal conclusion.

A formal child-privacy review is still required before release. It must inventory production analytics, error reporting, cookies and local storage, hosting logs, embedded/third-party requests, retention and deletion behavior, privacy notice language, parental/consent requirements, and jurisdiction-specific obligations. Any later cloud account, analytics, multiplayer, survey, or personalization feature requires a new review. The product should not claim COPPA or other legal compliance based only on the current local-first architecture.
...
- [x] Resolve mandatory-deck effects for Stunned, Lionfish invasion and attack-removal parity, Cookie Cutter's board-supply fallback, and per-attack Ensnare. Lionfish's additional "specialized Support card" route remains explicitly unavailable until authored card data identifies that Support.
